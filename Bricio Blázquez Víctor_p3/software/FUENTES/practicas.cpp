...
void score(){
	...
	for (unsigned i = 0; i < solucion.size(); i++){
		for (unsigned j = 0; j < solucion.size(); j++){
			//cout << solucion[i] << "  " << solucion[j] << " " << matrix.size() << endl;
...
/// i es el que está dentro y j es el que viene de fuera
...
void BusquedaMultiarranque(unsigned numero){
	double max = 0;
	...
	for (unsigned i = 0; i < numero; i++){
		...
		if (max < scoreTotal){
			for (unsigned i = 0; i < m; i++)
				solucionOptima.push_back(solucion[i]);
			max = scoreTotal;
		}
...
/// i es el que está dentro y j es el que viene de fuera
void IntercambioAJopo (int i, int j){
	for (unsigned k = 0; k < m; k++){
		if (solucion[k] == i )
			solucion[k] = j;
	}
...
void MutacionBestia(double indice){
	vector <int> a_meter, a_sacar;
	//bool cambio;
	//int pos;
	...
	int i = rand() % 2005;
  while (sol.size() < m * indice) {
    int a = rand(i) % (n - m);
    sol.insert(mundo[a]);
		//cout << a << " a " << mundo[a] << " rrrrr\n";
    i += rand() % 17;
  }

  for (auto it = sol.begin(); it != sol.end(); ++it)
    a_meter.push_back(*it);
...
cout << "SACAR\n";
	...
	for (unsigned i = 0; i < m * indice; i++){
		/*cambio =*/ IntercambioAJopo(a_sacar[i], a_meter[i]);

		/*cout << "i " << i << endl;
		pos = buscaPos(a_meter[i]);
		cout << "meter " << a_meter[i] << " pos  " << pos << endl;

		mundo.erase(mundo.begin() + pos);
		cout << "rrrr" << endl;
		if (cambio)
			mundo.push_back(a_sacar[i]);*/
	}
...
void BusquedaLocalReiterada(unsigned numero, double indice){
	double max;
	...
	for (unsigned i = 0; i < numero - 1; i++){
		MutacionBestia(indice);
		score();
		mundo.clear();
		crearMundo();

		//cout << "score 1 " << scoreTotal << endl;

		busquedaLocal(50000);

		//cout << "score 2 " << scoreTotal << endl;
...
double distanciaAcumulada2(int a){
	double suma = 0;
	bool found = false;
	...
	if (!found){
		for (unsigned i = 0; i < solucion.size(); i++){
			if (solucion[i] < a){
				suma += matrix[solucion[i]][a];
			}
			else{
				suma += matrix[a][solucion[i]];
			}
		}
	}
...
void algoritmoGRASP(int numero, double alfa){
	int max = 0, pos;
	double maxD, posibleMaxD, distanciaMin, dMAX = 0;
	vector<pair<double, int> > candidatos, candidatos2;
	...
	for (int j = 0; j < numero; j++){
		...
		while (solucion.size() < m){
			...
			for (int i = 0; i < n; i++){
				candidatos.emplace_back(distanciaAcumulada2(i), i);
			}

			for (unsigned i = 0; i < candidatos.size(); i++){
				if (candidatos[i].first != 0)
					candidatos2.push_back(candidatos[i]);
			}

			candidatos.clear();

			for (unsigned i = 0; i < candidatos2.size(); i++){
				candidatos.emplace_back(candidatos2[i]);
			}
			...
			for (unsigned i = 0; i < candidatos.size(); i++){
				cout << candidatos[i].first << " " << candidatos[i].second << "  ";
			}
			cout << endl;*/

			distanciaMin = candidatos[0].first + alfa * (candidatos[candidatos.size() - 1].first - candidatos[0].first);

			int a = candidatos.size() - 1;

			while (distanciaMin < candidatos[a].first){
				candidatos2.emplace_back(candidatos[a]);
				a--;
			}

			/*cout << "CANDIDATOS2" << endl;
			for (unsigned i = 0; i < candidatos2.size(); i++){
				cout << candidatos2[i].first << " " << candidatos2[i].second << "  ";
			}
			cout << endl;*/

			a = rand(a) % candidatos2.size();

			solucion.push_back(candidatos2[a].second);

			//cout << "A " << a << endl << candidatos2[a].second << endl;
...
void EnfriamientoSimulado(double mu, double fi){
	double tinicial, tfinal = 0.001, temperatura, beta;
	int max_vecinos = 10 * n, max_exitos = n, enfriamientos = 50000 / max_vecinos, vecinos = 0, exitos = 0;

	inicioRandom();
	score();
	crearMundo();

	tinicial = mu * scoreTotal/(-log(fi)/log(exp(1)));
	beta = (tinicial - tfinal) / (enfriamientos * tinicial * tfinal);
	...
	while(temperatura > tfinal){
		for (int i = 0; i < temperatura && exitos < max_exitos && vecinos < max_vecinos; i++){
			/*for (unsigned i = 0; i < m; i++)
				cout << solucion[i] << " ";
			cout << endl;*/

			int r = rand(temperatura) % m, s = rand(temperatura) % (n - m);

			//cout << solucion[r] << " " << mundo[s] << endl;

			r = solucion[r];
			s = mundo[s];

			IntercambioAJopo(r, s);

			/*for (int i = 0; i < m; i++)
				cout << solucion[i] << " ";
			cout << endl;*/
			int scoreViejo = scoreTotal;
			score();

			int desfase = scoreTotal - scoreViejo;
			mt19937 gen(temperatura);
			uniform_real_distribution<double> ran_u(0.0, 1.0);

			if (desfase > 0 || ran_u(gen) < exp(- desfase / temperatura)){
				exitos++;
...
void BusquedaEnfriamientoReiterada(unsigned numero, double indice){
	double max;

	inicioRandom();
	score();

	EnfriamientoSimulado(0.3, 0.3);
...
int main(int argc, char const *argv[]){
	...
	vector<double> /*tGrasp, dGrasp, tReiterada, dReiterada, tMulti, dMulti, tES, dES,*/ desviacion3, tiempo3, score3;
	//double maxD = 0;
	...
	for (int i = 1; i < 31; i++){
      ...
      score();
			...
			t0 = clock();
			BusquedaMultiarranque(25);
			t1 = clock();

			tiempo3.push_back((double(t1-t0)/CLOCKS_PER_SEC));
			score3.push_back(scoreTotal);

			cout << "Multiarranque" << i << endl;

			destruirLight();

			t0 = clock();
			BusquedaLocalReiterada(25, 0.1);
			t1 = clock();

			tiempo3.push_back((double(t1-t0)/CLOCKS_PER_SEC));
			score3.push_back(scoreTotal);

			cout << "Reiterada" << i << endl;

			destruirLight();

			t0 = clock();
			algoritmoGRASP(25, 0.3);
			t1 = clock();
			...
			cout << "Enfriamiento Simulado" << i << endl;
		...
		for (int j = 0; j < 4; j++){
			desviacion3.push_back((mejorValor[i - 1] - score3[4 * (i - 1) + j]) / mejorValor[i - 1]);
		}
	}
	...
	for (int i = 0; i < 30; i++){
		...
		for (int j = 0; j < 4; j++){
			switch (j) {
				case 0:
					cout << "		-Búsqueda Multiarranque: ";
				break;

				case 1:
					cout << "		-Búsqueda Local Reiterada: ";
				break;
			...
			cout <<  " 	Desviación: " << desviacion3[4 * i + j]  << " Tiempo: " << tiempo3[4 * i + j] << endl;
	}
}

	//input("Tablas/MDG-a_25_n2000_m200.txt");
  ...
  busquedaLocal(50000);
	...
	//algoritmoGRASP(25, 0.3);
	//EnfriamientoSimulado(0.3, 0.3);
	//BusquedaEnfriamientoReiterada(25, 0.1);
